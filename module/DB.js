...
module.exports = class DB {
    ...
    /**
     * Kulcs-érték párok alapján visszadja a keresett dokumentumot.
     * Használata: DB.findOne({name: 'vasaló'}).then(...)
     * @param {Object} where kulcs-érték párokban tartalmazza a szűrőfeltételt.
     * @return {Promise} null az érték ha nem talált, vagy objektum.
     */
    findOne(where) {
        return new Promise( (resolve, reject) => {
            Fsm.readPromise(this.filePath).then( (json) => {
    ...
    /**
     * Kulcs-érték párok alapján visszadja a keresett dokumentumokat.
     * Használata: DB.find({name: 'vasaló'}).then(...)
     * @param {Object} where kulcs-érték párokban tartalmazza a szűrőfeltételt.
     * @return {Promise} null az érték ha nem talált, vagy a lista.
     */
    find(where) {
        return new Promise( (resolve, reject) => {
            Fsm.readPromise(this.filePath).then( (json) => {
